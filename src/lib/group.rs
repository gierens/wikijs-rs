...
pub mod group_user_assign {
    use super::*;

    pub struct GroupUserAssign;

    pub const OPERATION_NAME: &str = "GroupUserAssign";
    pub const QUERY : & str = "mutation GroupUserAssign(\n  $groupId: Int!\n  $userId: Int!\n) {\n  groups {\n    assignUser(\n      groupId: $groupId\n      userId: $userId\n    ) {\n      responseResult {\n        succeeded\n        errorCode\n        slug\n        message\n      }\n    }\n  }\n}\n" ;
    ...
    #[derive(Deserialize)]
    pub struct Groups {
        #[serde(rename = "assignUser")]
        pub assign_user: Option<AssignUser>,
    }
    ...
    impl graphql_client::GraphQLQuery for GroupUserAssign {
        type Variables = Variables;
        type ResponseData = ResponseData;
        fn build_query(
            variables: Self::Variables,
...
pub fn group_user_assign(
    ...
    if let Some(data) = response_body.data {
        if let Some(groups) = data.groups {
            if let Some(assign_user) = groups.assign_user {
                if let Some(response_result) = assign_user.response_result {
                    if response_result.succeeded {
                        return Ok(());
                    } else {
                        return Err(
